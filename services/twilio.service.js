...
const validateFromNumber = () => {
  ...
  if (sandboxNumbers.includes(from)) {
    ...
    console.log(`📱 Using custom WhatsApp sender: ${formattedFrom}`);
    console.log("⚠️  Ensure this number is WhatsApp-enabled in your Twilio Console.");
  }
...
const validate = (to) => {
  if (!FROM) {
    throw new Error(
      "WhatsApp sender (FROM) is not configured. Check TWILIO_WHATSAPP_FROM in .env"
    );
  }
  if (!to) throw new Error("Recipient phone number is required");
};
...
// ─── Send plain text ─────────────────────────────────────────────────────────
// BUG FIX: Removed the redundant `normaliseWhatsappNumber(to)` call inside
// each send function. The controller already normalises before calling here,
// so calling it again was producing "whatsapp:whatsapp:+91..." double-prefixed
// numbers that Twilio rejected with a 21211 / channel error.
export const sendTextMessage = async (to, body) => {
  validate(to);
