...
/**
 * gmailService.js  —  MULTI-USER ISOLATED VERSION
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * SECURITY GUARANTEE
 * ═══════════════════════════════════════════════════════════════════════════
 * Every function that touches Gmail receives `crmUserId` — the MongoDB _id
 * of the logged-in CRM user, extracted from their verified JWT.
 *
 * Token lookup is ALWAYS:
 *   GmailToken.findOne({ crm_user_id: crmUserId, is_active: true })
 *
 * The client NEVER sends an email or userId to select whose mailbox to use.
 * Even if a malicious client sends a different email/userId in the request
 * body, the server ignores it completely.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * MULTI-USER FLOW (100–500 concurrent users)
 * ═══════════════════════════════════════════════════════════════════════════
 *  • Person A logs in → JWT contains A's _id
 *  • Person A calls /gmail/auth-status → checkAuth(A._id) → finds A's token
 *  • Person B logs in → JWT contains B's _id
 *  • Person B calls /gmail/auth-status → checkAuth(B._id) → finds B's token
 *  • No cross-contamination possible because DB query always filters by owner
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * KEY FIXES vs. original leaky code
 * ═══════════════════════════════════════════════════════════════════════════
 *  1. saveTokens: DELETE old rows, then INSERT fresh — no upsert ambiguity
 *  2. initializeGmailClient: in-memory cache keyed by crmUserId string
 *  3. generateAuthUrl: crmUserId embedded in JWT-signed state parameter
 *  4. exchangeCodeForTokens: state JWT verified → crmUserId extracted
 *  5. Every exported function requires crmUserId, never email from client
 */
...
// ─── PER-USER CLIENT CACHE ────────────────────────────────────────────────────
// Keyed by crmUserId.toString() — completely isolated per user
// Avoids hitting the DB on every API call for active sessions
const clientCache      = new Map(); // crmUserId → { client, email, expiresAt }
const CLIENT_CACHE_TTL = 45 * 60 * 1000; // 45 minutes

function getCacheKey(crmUserId) {
  return crmUserId ? crmUserId.toString() : null;
}

function invalidateClientCache(crmUserId) {
  const key = getCacheKey(crmUserId);
  if (key) clientCache.delete(key);
}

// Periodic cache cleanup to prevent memory leak with many users
setInterval(() => {
  const now = Date.now();
...
// ─── CORE: initializeGmailClient ─────────────────────────────────────────────
/**
 * Returns a ready-to-use Gmail API client for the given CRM user.
 * SECURITY: Only looks up the token bound to THIS user's ID.
 *           Never touches another user's token.
 */
export async function initializeGmailClient(crmUserId) {
  ...
  // SECURITY: Filter by crm_user_id — never by email
  ...
  if (!tokenDoc) {
    throw new Error("No Gmail account connected for this user. Please connect Gmail first.");
  }
  ...
  auth.setCredentials({
    access_token:  tokenDoc.access_token,
    refresh_token: tokenDoc.refresh_token,
    token_type:    tokenDoc.token_type || "Bearer",
    expiry_date:   new Date(tokenDoc.expiry_date).getTime(),
    scope:         tokenDoc.scope,
  });
  ...
  const expiryDate = new Date(tokenDoc.expiry_date);
  if (now >= expiryDate) {
    console.log(`🔄 Refreshing Gmail token for CRM user ${key}...`);
    try {
      const { credentials } = await auth.refreshAccessToken();
      // Save refreshed token back to DB (still bound to same crm_user_id)
      tokenDoc.access_token = credentials.access_token;
      tokenDoc.expiry_date  = new Date(credentials.expiry_date);
      if (credentials.refresh_token) tokenDoc.refresh_token = credentials.refresh_token;
      tokenDoc.last_connected = new Date();
      await tokenDoc.save();
      auth.setCredentials(credentials);
      console.log(`✅ Gmail token refreshed for CRM user ${key}`);
    } catch (err) {
      // Mark token as invalid so next request shows "reconnect" UI
      tokenDoc.is_active = false;
      await tokenDoc.save();
      invalidateClientCache(crmUserId);
  ...
  // Cache per user — independent of all other users
  clientCache.set(key, {
    client:    gmailClient,
  ...
  console.log(`✅ Gmail client ready for CRM user ${key} (${tokenDoc.email})`);
  return gmailClient;
}
 ...
 * The `state` parameter is a JWT signed with SECRET_KEY containing crmUserId.
 * When Google redirects back, we verify this state to know which CRM user
 * initiated the OAuth flow — no client can forge or tamper with it.
 */
export function generateAuthUrl(redirectUri, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required to generate auth URL");
  ...
  // Sign crmUserId into state — expires in 10 min (long enough for the OAuth flow)
 ...
 * Verifies the signed state JWT → extracts crmUserId → saves tokens bound to that user.
 * SECURITY: The crmUserId comes from the signed state, NOT from any request body.
 */
export async function exchangeCodeForTokens(code, redirectUri, state) {
  // 1. Verify the signed state to get the legitimate crmUserId
  let crmUserId;
  try {
    const payload = jwt.verify(state, process.env.SECRET_KEY);
    crmUserId     = payload.crmUserId;
    if (!crmUserId) throw new Error("Missing crmUserId in state");
  } catch (err) {
    throw new Error(`Invalid or expired OAuth state: ${err.message}. Please try connecting again.`);
  }
  ...
  // 3. Save tokens bound to this CRM user
  const result = await saveTokens(tokens, crmUserId);
  return { ...tokens, email: result.email, crmUserId };
}

// ─── SAVE TOKENS ─────────────────────────────────────────────────────────────
/**
 * Saves Gmail tokens to the database, bound to the given CRM user.
 *
 * ISOLATION GUARANTEE:
 *   - Deactivates ALL previous tokens for this user (and only this user)
 *   - Creates a new token row with crm_user_id = crmUserId
 *   - Two users can connect the same Gmail address (they get separate rows)
 *   - One user can reconnect (old row deactivated, new row created)
 *
 * PATTERN: deactivate-then-insert (not upsert) to avoid race conditions
 * and to ensure the new token always gets a fresh document.
 */
export async function saveTokens(tokens, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required to save tokens");

  // Verify the tokens by fetching the Gmail profile
  const tempClient = new google.auth.OAuth2(CLIENT_ID, CLIENT_SECRET, REDIRECT_URI);
  tempClient.setCredentials(tokens);
  const gmail   = google.gmail({ version: "v1", auth: tempClient });
  const profile = await gmail.users.getProfile({ userId: "me" });
  ...
  console.log(`📧 Saving Gmail tokens for CRM user ${crmUserId} (${email})`);

  const expiryDate = tokens.expiry_date
    ? new Date(tokens.expiry_date)
    : new Date(Date.now() + 3600000);

  // Step 1: Deactivate ALL previous tokens for THIS user only
  //         (does NOT affect any other user's tokens)
  await GmailToken.updateMany(
    { crm_user_id: crmUserId },
    { $set: { is_active: false, updated_at: new Date() } }
  );

  // Step 2: Insert a fresh token row for this user
  //         Using create() (not upsert) to avoid matching deactivated rows
  ...
  // Clear stale cache entry for this user
  invalidateClientCache(crmUserId);

  console.log(`✅ Gmail tokens saved for CRM user ${crmUserId} (${email})`);
  return { success: true, email };
}

// ─── CHECK AUTH ───────────────────────────────────────────────────────────────
/**
 * Checks if the given CRM user has an active, working Gmail connection.
 * Returns authenticated:false if not connected — does NOT look at any other user.
 */
export async function checkAuth(crmUserId) {
  if (!crmUserId) {
    return { authenticated: false, message: "Not logged in", email: null };
  }

  const uid = getCacheKey(crmUserId);

  // Fast path: use cached client info if available
  ...
  // DB lookup: only this user's token
  const tokenDoc = await GmailToken.findOne({
    crm_user_id: crmUserId,
    is_active:   true,
  });
  ...
  // Validate the token is still working with Google
  try {
    const gmailClient = await initializeGmailClient(crmUserId);
...
export async function disconnectGmail(crmUserId) {
  ...
  // Only deactivates THIS user's tokens
  await GmailToken.updateMany(
    { crm_user_id: crmUserId },
    { $set: { is_active: false, updated_at: new Date() } }
  );
...
// ─── GET CONNECTED EMAIL (display only) ──────────────────────────────────────
export async function getConnectedEmail(crmUserId) {
  const uid    = getCacheKey(crmUserId);
  const cached = clientCache.get(uid);
  if (cached) return cached.email;
  const tokenDoc = await GmailToken.findOne({ crm_user_id: crmUserId, is_active: true }).select("email");
  return tokenDoc ? tokenDoc.email : null;
}

// ─── LABEL COUNTS ─────────────────────────────────────────────────────────────
export async function getLabelCounts(crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail    = await initializeGmailClient(crmUserId);
  const labelIds = ["INBOX", "STARRED", "IMPORTANT", "SENT", "SPAM", "TRASH", "DRAFT"];
...
// ─── LIST THREADS ─────────────────────────────────────────────────────────────
export async function listThreads(maxResults = 20, pageToken = null, label = "INBOX", crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail     = await initializeGmailClient(crmUserId);
  const startTime = Date.now();
  ...
  const labelMap = {
    ...
    SENT: "in:sent",   SPAM: "in:spam",        TRASH: "in:trash",
    UNREAD: "is:unread in:inbox", DRAFTS: "in:drafts",
  };

  const params = {
    userId:     "me",
    maxResults: Math.min(maxResults, 50),
    q:          labelMap[label] || `label:${label.toLowerCase()}`,
  };
  ...
  const parsed = detailed
    ...
    .map((r) => {
      ...
      const lHeaders = lastMsg?.payload?.headers || headers;
      const labelIds = (lastMsg || firstMsg).labelIds || [];

      return {
        id:          thread.id,
        subject:     getHeader(headers, "Subject") || "(No Subject)",
        from:        getHeader(lHeaders, "From"),
        to:          getHeader(lHeaders, "To"),
        date:        getHeader(lHeaders, "Date"),
        timestamp:   lastMsg ? new Date(getHeader(lastMsg.payload?.headers || [], "Date")).getTime() : 0,
        snippet:     thread.snippet || "",
        unread:      labelIds.includes("UNREAD"),
        starred:     labelIds.includes("STARRED"),
        important:   labelIds.includes("IMPORTANT"),
        hasAttach:   labelIds.includes("HAS_ATTACHMENT"),
        labelIds,
  ...
  console.log(`⚡ Fetched ${parsed.length} threads for user ${crmUserId} in ${Date.now()-startTime}ms`);
  return {
    threads:           parsed,
...
// ─── GET THREAD ───────────────────────────────────────────────────────────────
export async function getThread(threadId, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail  = await initializeGmailClient(crmUserId);
...
// ─── MARK AS READ ─────────────────────────────────────────────────────────────
export async function markAsRead(threadId, read = true, crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
...
export async function sendEmailWithAttachments(
  ...
  attachments = [], inlineImages = [], crmUserId
) {
  if (!crmUserId) throw new Error("crmUserId required");
  ...
  toList.forEach((email) => {
    if (!validateEmailAddress(email)) throw new Error(`Invalid recipient email: ${email}`);
  });

  const gmail       = await initializeGmailClient(crmUserId);
  const fromProfile = await gmail.users.getProfile({ userId: "me" });
  const fromEmail   = fromProfile.data.emailAddress;

  const boundary  = `boundary_${Date.now()}_${Math.random().toString(36).substring(2)}`;
  const hasAttach = attachments.length > 0 || inlineImages.length > 0;
  const mimeType  = hasAttach ? "multipart/mixed" : "text/html";
  ...
  return { ...res.data, sendTime: Date.now() - startTime };
}
...
// ─── GET DRAFTS ───────────────────────────────────────────────────────────────
export async function getDrafts(maxResults = 20, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail = await initializeGmailClient(crmUserId);
  const res   = await gmail.users.drafts.list({ userId: "me", maxResults: Math.min(maxResults, 50) });
  const drafts = res.data.drafts || [];
  if (drafts.length === 0) return [];

  const detailed = await Promise.allSettled(
    drafts.slice(0, maxResults).map((d) =>
...
// ─── GET SINGLE DRAFT ─────────────────────────────────────────────────────────
export async function getDraft(draftId, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail = await initializeGmailClient(crmUserId);
  const res   = await gmail.users.drafts.get({ userId: "me", id: draftId, format: "full" });
...
// ─── DELETE DRAFT ─────────────────────────────────────────────────────────────
export async function deleteDraft(draftId, crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  await gmail.users.drafts.delete({ userId: "me", id: draftId });
  return { success: true };
}
...
export async function deleteThread(threadId, crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  await gmail.users.threads.delete({ userId: "me", id: threadId });
  return { success: true };
}

export async function deleteEmail(messageId, crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  await gmail.users.messages.delete({ userId: "me", id: messageId });
  return { success: true };
}
...
// ─── GET ATTACHMENT ───────────────────────────────────────────────────────────
export async function getAttachment(messageId, attachmentId, crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  const res   = await gmail.users.messages.attachments.get({ userId: "me", messageId, id: attachmentId });
  return { data: res.data.data, size: res.data.size };
}
...
// ─── THREAD ACTIONS ───────────────────────────────────────────────────────────
export async function starThread(threadId, star = true, crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  const res   = await gmail.users.threads.modify({
...
export async function bulkStarThreads(threadIds, star = true, crmUserId) {
  const gmail   = await initializeGmailClient(crmUserId);
  const results = await Promise.allSettled(
    threadIds.map((id) =>
      gmail.users.threads.modify({
  ...
  return {
    success: true,
    message: `${star ? "Starred" : "Unstarred"} ${results.filter((r) => r.status === "fulfilled").length} threads`,
    count:   results.filter((r) => r.status === "fulfilled").length,
  };
}

export async function markAsSpam(threadId, spam = true, crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  const res   = await gmail.users.threads.modify({
    userId: "me", id: threadId,
    requestBody: { addLabelIds: spam ? ["SPAM"] : [], removeLabelIds: spam ? [] : ["SPAM"] },
...
export async function markAsImportant(threadId, important = true, crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  const res   = await gmail.users.threads.modify({
    userId: "me", id: threadId,
    requestBody: { addLabelIds: important ? ["IMPORTANT"] : [], removeLabelIds: important ? [] : ["IMPORTANT"] },
...
export async function applyLabel(threadId, labelId, crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  const res   = await gmail.users.threads.modify({
    userId: "me", id: threadId,
    requestBody: { addLabelIds: [labelId] },
...
export async function moveToTrash(threadId, crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  const res   = await gmail.users.threads.trash({ userId: "me", id: threadId });
  return { success: true, ...res.data };
}

export async function bulkMoveToTrash(threadIds, crmUserId) {
  const gmail   = await initializeGmailClient(crmUserId);
  const results = await Promise.allSettled(
    threadIds.map((id) => gmail.users.threads.trash({ userId: "me", id }))
  );
  return {
    success: true,
    message: `Moved ${results.filter((r) => r.status === "fulfilled").length} threads to trash`,
    count:   results.filter((r) => r.status === "fulfilled").length,
  };
}

export async function bulkDeleteThreads(threadIds, crmUserId) {
  const gmail   = await initializeGmailClient(crmUserId);
  const results = await Promise.allSettled(
    threadIds.map((id) => gmail.users.threads.delete({ userId: "me", id }))
  );
  return {
    success: true,
    message: `Deleted ${results.filter((r) => r.status === "fulfilled").length} threads`,
    count:   results.filter((r) => r.status === "fulfilled").length,
  };
}

export async function getLabels(crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  const res   = await gmail.users.labels.list({ userId: "me" });
  return res.data.labels || [];
}
...
// ─── EMAIL SUGGESTIONS ────────────────────────────────────────────────────────
export async function getEmailSuggestions(query, limit = 10, crmUserId) {
  if (!crmUserId || !query || query.length < 2) return [];
  const gmail    = await initializeGmailClient(crmUserId);
  const emailSet = new Set();
...
export async function listAllThreads(crmUserId) {
  const gmail = await initializeGmailClient(crmUserId);
  const res   = await gmail.users.threads.list({ userId: "me", maxResults: 100, q: "in:inbox" });
  return (res.data.threads || []).map((t) => ({ id: t.id, snippet: t.snippet }));
}

export async function watchInbox(crmUserId) {
  await initializeGmailClient(crmUserId); // validates user has Gmail connected
  return { historyId: Date.now().toString() };
}
