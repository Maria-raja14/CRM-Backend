...
// ─── PER-USER CLIENT CACHE ────────────────────────────────────────────────────
// ✅ FIX: Cache key is now `${crmUserId}:${email}` — never shared across CRM users
...
// ─── CACHE KEY HELPER ─────────────────────────────────────────────────────────
function makeCacheKey(crmUserId, email) {
  return `${crmUserId}:${email.toLowerCase().trim()}`;
}

// ─── INIT CLIENT ──────────────────────────────────────────────────────────────
// ✅ FIX: Both crmUserId AND email required — prevents cross-user data leakage
export async function initializeGmailClient(email, crmUserId) {
  if (!email) throw new Error("email is required for Gmail client initialization");
  if (!crmUserId) throw new Error("crmUserId is required for Gmail client initialization");

  const normalEmail = email.toLowerCase().trim();
  const cacheKey = makeCacheKey(crmUserId, normalEmail);
  ...
  // ✅ FIX: Query by BOTH crm_user_id AND email — strict per-user isolation
  ...
  if (!tokenDoc) {
    throw new Error(`No Gmail token found for this account. Please connect Gmail first.`);
  }
  ...
  if (now > expiryDate) {
    console.log(`🔄 Refreshing token for ${normalEmail} (user: ${crmUserId})...`);
    try {
      const { credentials } = await auth.refreshAccessToken();
      tokenDoc.access_token = credentials.access_token;
      tokenDoc.expiry_date = new Date(credentials.expiry_date);
      ...
      await tokenDoc.save();
      auth.setCredentials(credentials);
      console.log(`✅ Token refreshed for ${normalEmail} (user: ${crmUserId})`);
    } catch (err) {
      tokenDoc.is_active = false;
      await tokenDoc.save();
  ...
  const gmailClient = google.gmail({ version: "v1", auth });
  clientCache.set(cacheKey, { client: gmailClient, expiresAt: Date.now() + CLIENT_CACHE_TTL });
  console.log(`✅ Gmail client ready for ${normalEmail} (user: ${crmUserId})`);
  return gmailClient;
}

function invalidateClientCache(crmUserId, email) {
  if (crmUserId && email) {
    clientCache.delete(makeCacheKey(crmUserId, email.toLowerCase().trim()));
  }
}
...
export function generateAuthUrl(redirectUri, crmUserId) {
  ...
  const client = new google.auth.OAuth2(CLIENT_ID, CLIENT_SECRET, redirectUri);
  // ✅ FIX: Embed crmUserId in state param so OAuth callback knows which user connected
  return client.generateAuthUrl({
    access_type: "offline",
    prompt: "consent",
    scope: scopes,
    state: crmUserId ? String(crmUserId) : undefined,
  });
}

// ─── SAVE TOKENS ─────────────────────────────────────────────────────────────
// ✅ FIX: crmUserId is now required — token always tied to specific CRM user
export async function saveTokens(tokens, crmUserId) {
  ...
  // ✅ FIX: Deactivate only for THIS CRM user's previous tokens for this Gmail
...
export async function exchangeCodeForTokens(code, redirectUri, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required for token exchange");
  const tempClient = new google.auth.OAuth2(CLIENT_ID, CLIENT_SECRET, redirectUri);
  const { tokens } = await tempClient.getToken(code);
...
// ─── CHECK AUTH ───────────────────────────────────────────────────────────────
// ✅ FIX: Always scoped to crmUserId
export async function checkAuth(email = null, crmUserId) {
  if (!crmUserId) return { authenticated: false, message: "CRM user not identified", email: null };

  const query = email
    ? { crm_user_id: crmUserId, email: email.toLowerCase().trim(), is_active: true }
  ...
  try {
    const gmail = await initializeGmailClient(tokenDoc.email, crmUserId);
    const profile = await gmail.users.getProfile({ userId: "me" });
    return {
      authenticated: true,
...
// ─── DISCONNECT ───────────────────────────────────────────────────────────────
export async function disconnectGmail(email, crmUserId) {
  if (!email) throw new Error("email required");
  if (!crmUserId) throw new Error("crmUserId required");
  const normalEmail = email.toLowerCase().trim();
  // ✅ FIX: Only deactivate tokens belonging to THIS CRM user
  await GmailToken.updateMany(
    { crm_user_id: crmUserId, email: normalEmail },
    { is_active: false }
  );
...
// ─── GET ALL ACTIVE ACCOUNTS FOR CRM USER ────────────────────────────────────
// ✅ FIX: Scoped to crmUserId — never returns other users' accounts
export async function getAllActiveAccounts(crmUserId) {
  if (!crmUserId) return [];
  const accounts = await GmailToken.find({ crm_user_id: crmUserId, is_active: true }).sort({
    last_connected: -1,
  });
  return accounts.map((a) => ({ email: a.email, last_connected: a.last_connected }));
}
...
// ─── GET LABEL COUNTS ─────────────────────────────────────────────────────────
export async function getLabelCounts(email, crmUserId) {
  if (!email) throw new Error("email required");
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail = await initializeGmailClient(email, crmUserId);
  ...
  console.log(`📊 Counts for ${email} (user: ${crmUserId}):`, counts);
  return counts;
}
...
// ─── LIST THREADS ─────────────────────────────────────────────────────────────
export async function listThreads(maxResults = 20, pageToken = null, label = "INBOX", email, crmUserId) {
  if (!email) throw new Error("email required");
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail = await initializeGmailClient(email, crmUserId);
  const startTime = Date.now();
    ...
    .map((result, idx) => {
      ...
      if (label === "SENT") {
        from =
          ...
          getHeader(firstHeaders, "To") ||
          "Unknown";
      } else {
        from =
          getHeader(lastHeaders, "From") ||
      ...
      const to = getHeader(firstHeaders, "To");
      const date =
        getHeader(lastHeaders, "Date") || getHeader(firstHeaders, "Date");
  ...
  threads.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
  console.log(
    `✅ ${email} (user: ${crmUserId}): ${threads.length} threads for ${label} in ${Date.now() - startTime}ms`
  );
...
// ─── GET THREAD ───────────────────────────────────────────────────────────────
export async function getThread(threadId, email, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail = await initializeGmailClient(email, crmUserId);
...
// ─── MARK AS READ ─────────────────────────────────────────────────────────────
export async function markAsRead(threadId, read = true, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  const res = await gmail.users.threads.modify({
    userId: "me",
    id: threadId,
...
export async function sendEmailWithAttachments(
  ...
  attachments = [], files = [], email, crmUserId
) {
  if (!email) throw new Error("email required");
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail = await initializeGmailClient(email, crmUserId);
  ...
  for (const att of allAttachments) {
    const b64 =
      att.content.replace(/\s/g, "").match(/.{1,76}/g)?.join(nl) || att.content;
    parts.push(
      `--${boundary}`,
      `Content-Type: ${att.mimetype}; name="${att.filename}"`,
      `Content-Disposition: attachment; filename="${att.filename}"`,
  ...
  const res = await gmail.users.messages.send({ userId: "me", requestBody: { raw } });
  const duration = (Date.now() - startTime) / 1000;
  console.log(`✅ Email sent for ${email} (user: ${crmUserId}) in ${duration.toFixed(2)}s`);
  return {
    success: true,
    id: res.data.id,
...
export async function sendEmail(to, subject, message, cc = "", bcc = "", attachments = [], files = [], email, crmUserId) {
  return sendEmailWithAttachments(to, subject, message, cc, bcc, attachments, files, email, crmUserId);
}

// ─── SAVE DRAFT ───────────────────────────────────────────────────────────────
export async function saveDraft(to, subject, message, cc = "", bcc = "", attachments = [], files = [], email, crmUserId) {
  if (!email) throw new Error("email required");
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail = await initializeGmailClient(email, crmUserId);
...
// ─── GET DRAFTS ───────────────────────────────────────────────────────────────
export async function getDrafts(maxResults = 20, email, crmUserId) {
  if (!email) throw new Error("email required");
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail = await initializeGmailClient(email, crmUserId);
  const listRes = await gmail.users.drafts.list({ userId: "me", maxResults });
...
// ─── GET DRAFT (single) ───────────────────────────────────────────────────────
export async function getDraft(draftId, email, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail = await initializeGmailClient(email, crmUserId);
  const res = await gmail.users.drafts.get({ userId: "me", id: draftId, format: "full" });
  const message = res.data.message;
...
// ─── GET ATTACHMENT ───────────────────────────────────────────────────────────
export async function getAttachment(messageId, attachmentId, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
...
// ─── DELETE ───────────────────────────────────────────────────────────────────
export async function deleteThread(threadId, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  await gmail.users.threads.delete({ userId: "me", id: threadId });
  return { success: true };
}

export async function deleteEmail(messageId, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  await gmail.users.messages.delete({ userId: "me", id: messageId });
  return { success: true };
}

export async function deleteDraft(draftId, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  await gmail.users.drafts.delete({ userId: "me", id: draftId });
  return { success: true };
}
...
// ─── THREAD ACTIONS ───────────────────────────────────────────────────────────
export async function starThread(threadId, star = true, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  const res = await gmail.users.threads.modify({
    userId: "me",
    id: threadId,
...
export async function bulkStarThreads(threadIds, star = true, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  const results = await Promise.allSettled(
    threadIds.map((id) =>
      gmail.users.threads.modify({
...
export async function markAsSpam(threadId, spam = true, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  const res = await gmail.users.threads.modify({
    userId: "me",
    id: threadId,
...
export async function markAsImportant(threadId, important = true, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  const res = await gmail.users.threads.modify({
    userId: "me",
    id: threadId,
...
export async function applyLabel(threadId, labelId, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  const res = await gmail.users.threads.modify({
    userId: "me",
    id: threadId,
...
export async function moveToTrash(threadId, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  const res = await gmail.users.threads.trash({ userId: "me", id: threadId });
  return { success: true, ...res.data };
}

export async function bulkMoveToTrash(threadIds, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  const results = await Promise.allSettled(
    threadIds.map((id) => gmail.users.threads.trash({ userId: "me", id }))
  );
...
export async function bulkDeleteThreads(threadIds, email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  const results = await Promise.allSettled(
    threadIds.map((id) => gmail.users.threads.delete({ userId: "me", id }))
  );
...
export async function getLabels(email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
  const res = await gmail.users.labels.list({ userId: "me" });
  return res.data.labels || [];
}
...
// ─── EMAIL SUGGESTIONS ────────────────────────────────────────────────────────
export async function getEmailSuggestions(query, limit = 10, email, crmUserId) {
  if (!email || !query || query.length < 2) return [];
  const gmail = await initializeGmailClient(email, crmUserId);
  const emailSet = new Set();
...
export async function listAllThreads(email, crmUserId) {
  const gmail = await initializeGmailClient(email, crmUserId);
...
export async function watchInbox(email, crmUserId) {
  await initializeGmailClient(email, crmUserId);
  return { historyId: Date.now().toString() };
}
...
function processEmailList(str) {
  ...
  return str
    .split(",")
    .map((e) => e.trim())
    .filter((e) => e && validateEmailAddress(e));
}

function extractAllEmails(str) {
  if (!str) return [];
  return (
    str.match(/([a-zA-Z0-9._+-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/gi) || []
  );
}