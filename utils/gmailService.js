...
// ─── PER-USER CLIENT CACHE (keyed by CRM user ID, not Gmail address) ─────────
// 🔒 Using crm_user_id as cache key so no cross-user leakage is possible
const clientCache   = new Map(); // crmUserId -> { client, expiresAt }
const CLIENT_CACHE_TTL = 45 * 60 * 1000; // 45 minutes
...
// ─── INIT CLIENT ──────────────────────────────────────────────────────────────
// 🔒 ALWAYS requires crmUserId — this is the primary security gate.
//    All DB lookups filter by crm_user_id so a user can only ever access
//    their own token.
export async function initializeGmailClient(crmUserId) {
  ...
  const cacheKey = crmUserId.toString();
  const cached   = clientCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) {
    return cached.client;
  }

  // 🔒 Filter by crm_user_id — only this CRM user's token is retrieved
  ...
  if (now > expiryDate) {
    console.log(`🔄 Refreshing token for CRM user ${crmUserId}...`);
    try {
      const { credentials } = await auth.refreshAccessToken();
      tokenDoc.access_token = credentials.access_token;
      tokenDoc.expiry_date  = new Date(credentials.expiry_date);
      ...
      await tokenDoc.save();
      auth.setCredentials(credentials);
      console.log(`✅ Token refreshed for CRM user ${crmUserId}`);
    } catch (err) {
      tokenDoc.is_active = false;
      await tokenDoc.save();
  ...
  console.log(`✅ Gmail client ready for CRM user ${crmUserId} (${tokenDoc.email})`);
  return gmailClient;
}
...
// ─── SAVE TOKENS ─────────────────────────────────────────────────────────────
// 🔒 crmUserId is mandatory — token is always bound to the CRM user
export async function saveTokens(tokens, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId is required to save tokens");
  ...
  const gmail      = google.gmail({ version: "v1", auth: tempClient });
  const profile    = await gmail.users.getProfile({ userId: "me" });
  const gmailEmail = profile.data.emailAddress.toLowerCase().trim();

  console.log(`📧 Saving tokens for CRM user ${crmUserId} → Gmail: ${gmailEmail}`);

  const expiryDate = tokens.expiry_date
    ? new Date(tokens.expiry_date)
    : new Date(Date.now() + 3600000);

  // 🔒 Deactivate only THIS CRM user's previous tokens for this Gmail address
  ...
  await GmailToken.findOneAndUpdate(
    { crm_user_id: crmUserId, email: gmailEmail },
    {
      crm_user_id: crmUserId,
      email:        gmailEmail,
      access_token: tokens.access_token,
  ...
  console.log(`✅ Tokens saved for CRM user ${crmUserId} (${gmailEmail})`);
...
// ─── DISCONNECT ───────────────────────────────────────────────────────────────
// 🔒 Only disconnects THIS CRM user's token
...
export async function listThreads(maxResults = 20, pageToken = null, label = "INBOX", crmUserId) {
  ...
  threads.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
  console.log(`✅ CRM user ${crmUserId}: ${threads.length} threads for ${label} in ${Date.now() - startTime}ms`);
...
export async function sendEmailWithAttachments(
  ...
  // 🔒 Get sender email from the DB token, not from request body
  const tokenDoc = await GmailToken.findOne({ crm_user_id: crmUserId, is_active: true });
  if (!tokenDoc) throw new Error("No connected Gmail account found");
  ...
  const raw = Buffer.from(parts.join(nl), "utf8").toString("base64")
    .replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");

  const startTime = Date.now();
  const res       = await gmail.users.messages.send({ userId: "me", requestBody: { raw } });
  const duration  = (Date.now() - startTime) / 1000;
  console.log(`✅ Email sent for CRM user ${crmUserId} in ${duration.toFixed(2)}s`);
  return { success: true, id: res.data.id, threadId: res.data.threadId, labelIds: res.data.labelIds || [], sendTime: duration };
}
...
// ─── GET CONNECTED ACCOUNT FOR THIS CRM USER ─────────────────────────────────
// 🔒 Only returns the account belonging to this CRM user
export async function getConnectedAccount(crmUserId) {
  if (!crmUserId) return null;
  const tokenDoc = await GmailToken.findOne({ crm_user_id: crmUserId, is_active: true })
    .sort({ last_connected: -1 });
  if (!tokenDoc) return null;
  return { email: tokenDoc.email, last_connected: tokenDoc.last_connected };
}
