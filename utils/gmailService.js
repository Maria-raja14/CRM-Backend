...
/**
 * gmailService.js  —  SECURITY-HARDENED VERSION
 *
 * ROOT CAUSE OF DATA LEAK (original code):
 *   Every API call accepted `email` from req.body / req.query / req.headers.
 *   Any authenticated CRM user could pass ANY email address and read, delete,
 *   or send mail belonging to someone else.
 *
 * FIX STRATEGY:
 *   1.  All service functions accept `crmUserId` (from the JWT, never from
 *       the client request body).
 *   2.  Token lookup is ALWAYS by `{ crm_user_id, is_active: true }`.
 *       The email stored in the DB is only used to build the OAuth client —
 *       it is never trusted from the client.
 *   3.  The OAuth callback stores the token WITH the crmUserId extracted from
 *       the `state` parameter that was embedded in the auth URL at request time
 *       (signed with JWT so it cannot be tampered with).
 *   4.  Client-side cache keys still use the email for display purposes, but
 *       the server never lets a client dictate which token to load.
 */
...
// ─── PER-USER CLIENT CACHE (keyed by crmUserId string) ───────────────────────
...
function invalidateClientCache(crmUserId) {
  if (crmUserId) clientCache.delete(crmUserId.toString());
}

// ─── INIT CLIENT  (SECURITY: lookup by crmUserId, not by email) ──────────────
export async function initializeGmailClient(crmUserId) {
  if (!crmUserId) throw new Error("crmUserId is required");

  const uid    = crmUserId.toString();
  const cached = clientCache.get(uid);
  if (cached && cached.expiresAt > Date.now()) {
    return cached.client;
  }

  // SECURITY: only load the token that belongs to THIS CRM user
  const tokenDoc = await GmailToken.findOne({ crm_user_id: uid, is_active: true });
  if (!tokenDoc) {
  ...
  console.log(`✅ Gmail client ready for user ${uid} (${tokenDoc.email})`);
  return gmailClient;
}

// Helper: get the email address stored for this CRM user (for display only)
...
// ─── AUTH URL  (embed signed crmUserId in `state`) ───────────────────────────
export function generateAuthUrl(redirectUri, crmUserId) {
  ...
  // Sign the state so it cannot be forged by a client
  const state  = jwt.sign({ crmUserId: crmUserId.toString() }, process.env.SECRET_KEY, { expiresIn: "10m" });
  const client = new google.auth.OAuth2(CLIENT_ID, CLIENT_SECRET, redirectUri);
...
// ─── SAVE TOKENS  (SECURITY: bind token to verified crmUserId) ───────────────
export async function saveTokens(tokens, crmUserId) {
  ...
  console.log(`📧 Saving tokens for user ${crmUserId} (${email})`);

  const expiryDate = tokens.expiry_date
    ? new Date(tokens.expiry_date)
    : new Date(Date.now() + 3600000);

  // SECURITY: Deactivate any previous tokens for THIS user only
  await GmailToken.updateMany({ crm_user_id: crmUserId }, { is_active: false });
  ...
  console.log(`✅ Tokens saved for user ${crmUserId} (${email})`);
  return { success: true, email };
}

// ─── EXCHANGE CODE  (verify signed state → extract crmUserId) ────────────────
...
export async function checkAuth(crmUserId) {
  if (!crmUserId) return { authenticated: false, message: "Not logged in", email: null };

  const tokenDoc = await GmailToken.findOne({ crm_user_id: crmUserId, is_active: true });
  if (!tokenDoc) return { authenticated: false, message: "No Gmail account connected", email: null };
...
export async function listThreads(maxResults = 20, pageToken = null, label = "INBOX", crmUserId) {
  ...
  if (label === "DRAFTS") {
    const listRes = await gmail.users.drafts.list({
      userId: "me", maxResults, pageToken: pageToken || undefined,
    });
    const drafts = listRes.data.drafts || [];
    if (!drafts.length) return { threads: [], nextPageToken: null, resultSizeEstimate: 0 };
    ...
    const results = await Promise.allSettled(
      drafts.map((draft) =>
        gmail.users.messages.get({
          userId: "me", id: draft.message.id, format: "metadata",
          metadataHeaders: ["Subject", "From", "To", "Date"],
        })
      )
    );
  ...
  switch (label) {
    case "INBOX":     params.labelIds = ["INBOX"]; break;
    case "UNREAD":    params.q = "is:unread"; params.includeSpamTrash = false; break;
    case "STARRED":   params.labelIds = ["STARRED"]; break;
    case "IMPORTANT": params.labelIds = ["IMPORTANT"]; break;
    case "SENT":      params.labelIds = ["SENT"]; break;
    case "SPAM":      params.labelIds = ["SPAM"]; break;
  ...
  const threads = results.map((result, idx) => {
    ...
    if (result.status !== "fulfilled") {
      return {
        id: stub.id, snippet: stub.snippet || "", subject: "(No Subject)",
        from: "Unknown", to: "", date: "", timestamp: 0,
        unread: false, starred: false, important: false,
        spam: false, trash: false, isDraft: false, messagesCount: 0,
      };
    }
    ...
    const subject = getHeader(firstHeaders, "Subject") || getHeader(lastHeaders, "Subject") || "(No Subject)";
    const from    = label === "SENT"
      ? (getHeader(lastHeaders, "To") || getHeader(firstHeaders, "To") || "Unknown")
      : (getHeader(lastHeaders, "From") || getHeader(firstHeaders, "From") || "Unknown");
    const to      = getHeader(firstHeaders, "To");
    const date    = getHeader(lastHeaders, "Date") || getHeader(firstHeaders, "Date");
  ...
  threads.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
  console.log(`✅ User ${crmUserId}: ${threads.length} threads for ${label} in ${Date.now() - startTime}ms`);
...
export async function sendEmailWithAttachments(
  to, subject, message, cc = "", bcc = "", attachments = [], files = [], crmUserId
) {
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail     = await initializeGmailClient(crmUserId);
  const fromEmail = await getConnectedEmail(crmUserId);
  ...
  const res       = await gmail.users.messages.send({ userId: "me", requestBody: { raw } });
  const duration  = (Date.now() - startTime) / 1000;
  console.log(`✅ Email sent for user ${crmUserId} in ${duration.toFixed(2)}s`);
  return { success: true, id: res.data.id, threadId: res.data.threadId, labelIds: res.data.labelIds || [], sendTime: duration };
}

export async function sendEmail(to, subject, message, cc = "", bcc = "", attachments = [], files = [], crmUserId) {
  return sendEmailWithAttachments(to, subject, message, cc, bcc, attachments, files, crmUserId);
}

// ─── SAVE DRAFT ───────────────────────────────────────────────────────────────
export async function saveDraft(to, subject, message, cc = "", bcc = "", attachments = [], files = [], crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required");
  const gmail     = await initializeGmailClient(crmUserId);
  const fromEmail = await getConnectedEmail(crmUserId);
  ...
  const boundary = `gmailbnd_${Date.now()}`;
  const nl       = "\r\n";
  const parts    = ["MIME-Version: 1.0", `To: ${to}`, `From: ${fromEmail}`, `Subject: ${subject || "(No Subject)"}`];
  if (cc)  parts.push(`Cc: ${cc}`);
  if (bcc) parts.push(`Bcc: ${bcc}`);
  parts.push(`Content-Type: multipart/mixed; boundary="${boundary}"`, "");
  parts.push(`--${boundary}`, `Content-Type: text/plain; charset="UTF-8"`, "Content-Transfer-Encoding: quoted-printable", "", message || " ", "");
...
export async function watchInbox(crmUserId) {
  await initializeGmailClient(crmUserId);
  return { historyId: Date.now().toString() };
}
