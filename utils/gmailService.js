...
// ─── PER-USER CLIENT CACHE ────────────────────────────────────────────────────
// ✅ SECURITY: Cache key is crmUserId (ObjectId string), NOT email.
//    This ensures two CRM users who happen to share a Gmail address
//    each get their own isolated client.
...
// ─── INIT CLIENT ──────────────────────────────────────────────────────────────
// ✅ SECURITY: Always look up token by (crmUserId + email) — never email alone.
export async function initializeGmailClient(crmUserId, email) {
  if (!crmUserId) throw new Error("crmUserId is required");
  if (!email) throw new Error("email is required");

  const crmUserIdStr = crmUserId.toString();
  const normalEmail = email.toLowerCase().trim();
  const cacheKey = crmUserIdStr;
  ...
  // ✅ SECURITY: Must match BOTH crm_user_id AND email AND is_active
  ...
  if (!tokenDoc) {
    throw new Error(`No Gmail token found for this user. Please connect Gmail first.`);
  }
  ...
  if (now > expiryDate) {
    console.log(`🔄 Refreshing token for user ${crmUserIdStr}...`);
    try {
      const { credentials } = await auth.refreshAccessToken();
      tokenDoc.access_token = credentials.access_token;
      tokenDoc.expiry_date = new Date(credentials.expiry_date);
      ...
      await tokenDoc.save();
      auth.setCredentials(credentials);
      console.log(`✅ Token refreshed for user ${crmUserIdStr}`);
    } catch (err) {
      tokenDoc.is_active = false;
      await tokenDoc.save();
  ...
  console.log(`✅ Gmail client ready for CRM user ${crmUserIdStr}`);
  return gmailClient;
}
...
export function generateAuthUrl(redirectUri, crmUserId) {
  ...
  const client = new google.auth.OAuth2(CLIENT_ID, CLIENT_SECRET, redirectUri);
  // ✅ SECURITY: Embed crmUserId in the OAuth state param so we know
  //    which CRM user is completing the OAuth flow on callback.
  return client.generateAuthUrl({
    access_type: "offline",
    prompt: "consent",
    scope: scopes,
    state: crmUserId.toString(), // passed back in ?state= on callback
  });
}

// ─── SAVE TOKENS ─────────────────────────────────────────────────────────────
// ✅ SECURITY: Always saves with crm_user_id — scoped to the authenticating user.
export async function saveTokens(tokens, crmUserId) {
  ...
  console.log(`📧 Saving tokens for CRM user ${crmUserId}, Gmail: ${email}`);

  const expiryDate = tokens.expiry_date
    ? new Date(tokens.expiry_date)
    : new Date(Date.now() + 3600000);

  // ✅ SECURITY: Deactivate only THIS user's existing tokens — not other users' tokens
  ...
  console.log(`✅ Tokens saved for CRM user ${crmUserId}, Gmail: ${email}`);
  return { success: true, email };
}
...
// ─── CHECK AUTH ───────────────────────────────────────────────────────────────
// ✅ SECURITY: Only checks tokens belonging to the authenticated CRM user.
export async function checkAuth(crmUserId) {
  if (!crmUserId) return { authenticated: false, message: "Not authenticated", email: null };
...
// ─── DISCONNECT ───────────────────────────────────────────────────────────────
// ✅ SECURITY: Only disconnects THIS user's token — never touches other users.
export async function disconnectGmail(crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required");
  // Only deactivate tokens owned by this CRM user
  await GmailToken.updateMany({ crm_user_id: crmUserId }, { is_active: false });
  invalidateClientCache(crmUserId);
  console.log(`✅ Gmail disconnected for CRM user ${crmUserId}`);
  return { success: true, message: "Gmail disconnected successfully" };
}

// ─── GET CONNECTED EMAIL FOR USER ─────────────────────────────────────────────
...
// ✅ SECURITY: Only returns accounts belonging to this CRM user
export async function getActiveAccountsForUser(crmUserId) {
  if (!crmUserId) return [];
