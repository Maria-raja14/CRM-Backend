...
// ─── PER-USER CLIENT CACHE ────────────────────────────────────────────────────
// KEY: `${crmUserId}:${email}` → guarantees strict per-CRM-user isolation
// Even if two CRM users link the same Gmail address they get separate cache entries
...
// ─── CACHE KEY ────────────────────────────────────────────────────────────────
function cacheKey(crmUserId, email) {
  return `${crmUserId.toString()}:${email.toLowerCase().trim()}`;
}

// ─── INIT CLIENT ──────────────────────────────────────────────────────────────
// ✅ ALWAYS requires BOTH crmUserId AND email — no ambiguous lookups possible
export async function initializeGmailClient(crmUserId, email) {
  if (!crmUserId) throw new Error("crmUserId is required for Gmail client initialization");
  if (!email)     throw new Error("email is required for Gmail client initialization");

  const normalEmail  = email.toLowerCase().trim();
  const key          = cacheKey(crmUserId, normalEmail);
  ...
  // ✅ Lookup by BOTH crmUserId AND email — never leaks between users
  ...
  if (!tokenDoc) {
    throw new Error(
      `No Gmail token found for CRM user ${crmUserId} / ${email}. Please connect Gmail first.`
    );
  }
  ...
  if (now > expiryDate) {
    console.log(`🔄 Refreshing token for CRM user ${crmUserId} / ${normalEmail}...`);
    try {
      const { credentials } = await auth.refreshAccessToken();
      tokenDoc.access_token  = credentials.access_token;
      tokenDoc.expiry_date   = new Date(credentials.expiry_date);
      ...
      await tokenDoc.save();
      auth.setCredentials(credentials);
      console.log(`✅ Token refreshed for CRM user ${crmUserId} / ${normalEmail}`);
    } catch (err) {
      tokenDoc.is_active = false;
      await tokenDoc.save();
  ...
  console.log(`✅ Gmail client ready for CRM user ${crmUserId} / ${normalEmail}`);
  return gmailClient;
}

function invalidateClientCache(crmUserId, email) {
  if (crmUserId && email) {
    clientCache.delete(cacheKey(crmUserId, email));
  }
}
...
export function generateAuthUrl(redirectUri, crmUserId) {
  ...
  return client.generateAuthUrl({
    ...
    prompt:      "consent",
    scope:       scopes,
    // ✅ Encode crmUserId in state so the OAuth callback knows who is connecting
    state:       Buffer.from(JSON.stringify({ crmUserId: crmUserId.toString() })).toString("base64"),
  });
}

// ─── SAVE TOKENS ─────────────────────────────────────────────────────────────
// ✅ crmUserId is mandatory — tokens are always scoped to a CRM user
export async function saveTokens(tokens, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId is required to save Gmail tokens");
  ...
  console.log(`📧 Saving tokens for CRM user ${crmUserId} / ${email}`);

  const expiryDate = tokens.expiry_date
    ? new Date(tokens.expiry_date)
    : new Date(Date.now() + 3600000);

  // ✅ Deactivate ONLY this CRM user's previous tokens for this email — never touches other users
  await GmailToken.updateMany({ crm_user_id: crmUserId, email }, { is_active: false });

  await GmailToken.findOneAndUpdate(
    { crm_user_id: crmUserId, email },
    {
      crm_user_id:  crmUserId,
      email,
      access_token: tokens.access_token,
  ...
  invalidateClientCache(crmUserId, email);
  console.log(`✅ Tokens saved for CRM user ${crmUserId} / ${email}`);
  return { success: true, email };
}

// ─── EXCHANGE CODE ────────────────────────────────────────────────────────────
export async function exchangeCodeForTokens(code, redirectUri, crmUserId) {
  if (!crmUserId) throw new Error("crmUserId is required to exchange tokens");
  const tempClient    = new google.auth.OAuth2(CLIENT_ID, CLIENT_SECRET, redirectUri);
  const { tokens }    = await tempClient.getToken(code);
...
// ─── CHECK AUTH ───────────────────────────────────────────────────────────────
// ✅ Always scoped to the requesting CRM user
export async function checkAuth(crmUserId, email = null) {
  if (!crmUserId) return { authenticated: false, message: "Not logged in to CRM", email: null };
  ...
  try {
    const gmail   = await initializeGmailClient(crmUserId, tokenDoc.email);
    const profile = await gmail.users.getProfile({ userId: "me" });
    return {
      authenticated: true,
    ...
    tokenDoc.is_active = false;
    await tokenDoc.save();
    invalidateClientCache(crmUserId, tokenDoc.email);
    return { authenticated: false, message: `Auth failed: ${err.message}`, email: tokenDoc.email };
  }
}

// ─── DISCONNECT ───────────────────────────────────────────────────────────────
export async function disconnectGmail(crmUserId, email) {
  if (!crmUserId) throw new Error("crmUserId required");
  if (!email)     throw new Error("email required");
  const normalEmail = email.toLowerCase().trim();
  // ✅ Only deactivates THIS user's token — never touches another CRM user's tokens
  await GmailToken.updateMany({ crm_user_id: crmUserId, email: normalEmail }, { is_active: false });
  invalidateClientCache(crmUserId, normalEmail);
  return { success: true, message: `Gmail disconnected for ${normalEmail}` };
}

// ─── GET LABEL COUNTS ─────────────────────────────────────────────────────────
export async function getLabelCounts(crmUserId, email) {
  if (!crmUserId) throw new Error("crmUserId required");
  if (!email)     throw new Error("email required");
  const gmail    = await initializeGmailClient(crmUserId, email);
  const labelIds = ["INBOX", "STARRED", "IMPORTANT", "SENT", "SPAM", "TRASH", "DRAFT"];
  ...
  console.log(`📊 Counts for CRM user ${crmUserId} / ${email}:`, counts);
  return counts;
}
...
// ─── LIST THREADS ─────────────────────────────────────────────────────────────
export async function listThreads(maxResults = 20, pageToken = null, label = "INBOX", crmUserId, email) {
  if (!crmUserId) throw new Error("crmUserId required");
  if (!email)     throw new Error("email required");
  const gmail     = await initializeGmailClient(crmUserId, email);
  const startTime = Date.now();
  ...
  threads.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
  console.log(`✅ CRM user ${crmUserId} / ${email}: ${threads.length} threads for ${label} in ${Date.now() - startTime}ms`);
...
// ─── GET THREAD ───────────────────────────────────────────────────────────────
export async function getThread(threadId, crmUserId, email) {
  if (!crmUserId) throw new Error("crmUserId required");
  if (!email)     throw new Error("email required");
  const gmail = await initializeGmailClient(crmUserId, email);
  const res   = await gmail.users.threads.get({ userId: "me", id: threadId, format: "full" });
...
// ─── MARK AS READ ─────────────────────────────────────────────────────────────
export async function markAsRead(threadId, read = true, crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  const res   = await gmail.users.threads.modify({
    userId:      "me",
    id:          threadId,
...
// ─── SEND EMAIL ───────────────────────────────────────────────────────────────
export async function sendEmailWithAttachments(to, subject, message, cc = "", bcc = "", attachments = [], files = [], crmUserId, email) {
  if (!crmUserId) throw new Error("crmUserId required");
  if (!email)     throw new Error("email required");
  const gmail     = await initializeGmailClient(crmUserId, email);
  const fromEmail = email;
  ...
  const res       = await gmail.users.messages.send({ userId: "me", requestBody: { raw } });
  const duration  = (Date.now() - startTime) / 1000;
  console.log(`✅ Email sent for CRM user ${crmUserId} / ${email} in ${duration.toFixed(2)}s`);
  return { success: true, id: res.data.id, threadId: res.data.threadId, labelIds: res.data.labelIds || [], sendTime: duration };
}

export async function sendEmail(to, subject, message, cc = "", bcc = "", attachments = [], files = [], crmUserId, email) {
  return sendEmailWithAttachments(to, subject, message, cc, bcc, attachments, files, crmUserId, email);
}

// ─── SAVE DRAFT ───────────────────────────────────────────────────────────────
export async function saveDraft(to, subject, message, cc = "", bcc = "", attachments = [], files = [], crmUserId, email) {
  if (!crmUserId) throw new Error("crmUserId required");
  if (!email)     throw new Error("email required");
  const gmail   = await initializeGmailClient(crmUserId, email);
  const allAtts = [...attachments];
...
// ─── GET DRAFTS ───────────────────────────────────────────────────────────────
export async function getDrafts(maxResults = 20, crmUserId, email) {
  if (!crmUserId) throw new Error("crmUserId required");
  if (!email)     throw new Error("email required");
  const gmail   = await initializeGmailClient(crmUserId, email);
  const listRes = await gmail.users.drafts.list({ userId: "me", maxResults });
...
// ─── GET DRAFT (single) ───────────────────────────────────────────────────────
export async function getDraft(draftId, crmUserId, email) {
  if (!crmUserId) throw new Error("crmUserId required");
  if (!email)     throw new Error("email required");
  const gmail   = await initializeGmailClient(crmUserId, email);
  const res     = await gmail.users.drafts.get({ userId: "me", id: draftId, format: "full" });
...
// ─── GET ATTACHMENT ───────────────────────────────────────────────────────────
export async function getAttachment(messageId, attachmentId, crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  const res   = await gmail.users.messages.attachments.get({ userId: "me", messageId, id: attachmentId });
  return { data: res.data.data, size: res.data.size };
}
...
// ─── DELETE ───────────────────────────────────────────────────────────────────
export async function deleteThread(threadId, crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  await gmail.users.threads.delete({ userId: "me", id: threadId });
  return { success: true };
}

export async function deleteEmail(messageId, crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  await gmail.users.messages.delete({ userId: "me", id: messageId });
  return { success: true };
}

export async function deleteDraft(draftId, crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  await gmail.users.drafts.delete({ userId: "me", id: draftId });
  return { success: true };
}
...
// ─── THREAD ACTIONS ───────────────────────────────────────────────────────────
export async function starThread(threadId, star = true, crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  const res   = await gmail.users.threads.modify({
    userId:      "me",
    id:          threadId,
...
export async function bulkStarThreads(threadIds, star = true, crmUserId, email) {
  const gmail   = await initializeGmailClient(crmUserId, email);
  const results = await Promise.allSettled(
    threadIds.map((id) =>
      gmail.users.threads.modify({
...
export async function markAsSpam(threadId, spam = true, crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  const res   = await gmail.users.threads.modify({
    userId:      "me",
    id:          threadId,
...
export async function markAsImportant(threadId, important = true, crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  const res   = await gmail.users.threads.modify({
    userId:      "me",
    id:          threadId,
...
export async function applyLabel(threadId, labelId, crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  const res   = await gmail.users.threads.modify({
    userId:      "me",
    id:          threadId,
...
export async function moveToTrash(threadId, crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  const res   = await gmail.users.threads.trash({ userId: "me", id: threadId });
  return { success: true, ...res.data };
}

export async function bulkMoveToTrash(threadIds, crmUserId, email) {
  const gmail   = await initializeGmailClient(crmUserId, email);
  const results = await Promise.allSettled(
    threadIds.map((id) => gmail.users.threads.trash({ userId: "me", id }))
  );
...
export async function bulkDeleteThreads(threadIds, crmUserId, email) {
  const gmail   = await initializeGmailClient(crmUserId, email);
  const results = await Promise.allSettled(
    threadIds.map((id) => gmail.users.threads.delete({ userId: "me", id }))
  );
...
export async function getLabels(crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  const res   = await gmail.users.labels.list({ userId: "me" });
  return res.data.labels || [];
}
...
// ─── EMAIL SUGGESTIONS ────────────────────────────────────────────────────────
export async function getEmailSuggestions(query, limit = 10, crmUserId, email) {
  if (!crmUserId || !email || !query || query.length < 2) return [];
  const gmail    = await initializeGmailClient(crmUserId, email);
  const emailSet = new Set();
...
// ✅ Only returns accounts belonging to the requesting CRM user
export async function getAllActiveAccounts(crmUserId) {
  if (!crmUserId) throw new Error("crmUserId required");
  const accounts = await GmailToken.find({ crm_user_id: crmUserId, is_active: true }).sort({ last_connected: -1 });
  return accounts.map((a) => ({ email: a.email, last_connected: a.last_connected }));
}

export async function listAllThreads(crmUserId, email) {
  const gmail = await initializeGmailClient(crmUserId, email);
  const res   = await gmail.users.threads.list({ userId: "me", maxResults: 100, q: "in:inbox" });
  return (res.data.threads || []).map((t) => ({ id: t.id, snippet: t.snippet }));
}

export async function watchInbox(crmUserId, email) {
  await initializeGmailClient(crmUserId, email);
  return { historyId: Date.now().toString() };
}
