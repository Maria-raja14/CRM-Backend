...
const gmailTokenSchema = new mongoose.Schema({
  // ✅ PRIMARY KEY: crm_user_id — every token is OWNED by a specific CRM user
  crm_user_id: {
    type: mongoose.Schema.Types.ObjectId,
    ref: "User",
    required: true,
  ...
  // Gmail email address (can have multiple CRM users link the same Gmail — allowed)
  email: {
    type: String,
    required: true,
    lowercase: true,
...
// ✅ Compound unique index: one active token per (crm_user_id + email) pair
