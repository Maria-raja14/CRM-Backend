...
import express from "express";
import multer from "multer";
import { protect } from "../middlewares/auth.middleware.js"; // ✅ FIX: All routes protected
import {
  generateAuthUrl,
  exchangeCodeForTokens,
  listThreads,
...
// ✅ FIX: All routes use protect middleware — req.user is always the authenticated CRM user.
// We NEVER trust client-supplied identity for sensitive operations.
// The crmUserId comes from the verified JWT token, not query/body params.
...
// ✅ FIX: Extract CRM user ID from JWT (set by protect middleware)
function getCrmUserId(req) {
  return req.user?._id?.toString() || null;
}

// ═══════════════════════════════════════
// ACCOUNTS — scoped to authenticated user
// ═══════════════════════════════════════
router.get("/accounts", protect, async (req, res) => {
  try {
    const crmUserId = getCrmUserId(req);
    const accounts = await getAllActiveAccounts(crmUserId);
    res.json({ success: true, accounts, current: accounts[0] || null });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
// COUNTS
// ═══════════════════════════════════════
router.get("/all-counts", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    const counts = await getLabelCounts(email, crmUserId);
    res.json({ success: true, counts });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
router.get("/threads", protect, async (req, res) => {
  try {
    ...
    const pageToken = req.query.pageToken || null;
    const label = req.query.label || "INBOX";
    const result = await listThreads(maxResults, pageToken, label, email, crmUserId);
    res.set("Cache-Control", "private, max-age=30");
    res.json({
      success: true,
      data: result.threads,
...
router.get("/all-threads", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    const threads = await listAllThreads(email, crmUserId);
    res.json({ success: true, data: threads });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.get("/thread/:id", protect, async (req, res) => {
  try {
    ...
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    const thread = await getThread(req.params.id, email, crmUserId);
    res.json({ success: true, data: thread });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.delete("/thread/:id", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    await deleteThread(req.params.id, email, crmUserId);
    res.json({ success: true, message: "Thread deleted" });
  } catch (err) {
    if (err.message?.includes("insufficientPermissions"))
...
router.post("/thread/:id/read", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    await markAsRead(req.params.id, req.body.read !== false, email, crmUserId);
    res.json({
      success: true,
      message: `Marked as ${req.body.read !== false ? "read" : "unread"}`,
    });
...
router.post("/thread/:id/star", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    await starThread(req.params.id, req.body.star !== false, email, crmUserId);
    res.json({
      success: true,
      message: `Thread ${req.body.star !== false ? "starred" : "unstarred"}`,
    });
...
router.post("/thread/:id/spam", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    await markAsSpam(req.params.id, req.body.spam !== false, email, crmUserId);
    res.json({ success: true, message: "Spam status updated" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/thread/:id/important", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    await markAsImportant(req.params.id, req.body.important !== false, email, crmUserId);
    res.json({ success: true, message: "Important status updated" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/thread/:id/trash", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    await moveToTrash(req.params.id, email, crmUserId);
    res.json({ success: true, message: "Thread moved to trash" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/thread/:id/label", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    await applyLabel(req.params.id, req.body.labelId, email, crmUserId);
    res.json({ success: true, message: "Label applied" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/bulk-delete", protect, async (req, res) => {
  try {
    ...
    if (!threadIds?.length)
      return res.status(400).json({ success: false, error: "No thread IDs" });
    const result = permanent
      ? await bulkDeleteThreads(threadIds, email, crmUserId)
      : await bulkMoveToTrash(threadIds, email, crmUserId);
    res.json({ success: true, message: result.message, data: result });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/bulk-star", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    ...
    if (!threadIds?.length)
      return res.status(400).json({ success: false, error: "No thread IDs" });
    const result = await bulkStarThreads(threadIds, star !== false, email, crmUserId);
    res.json({ success: true, message: result.message, data: result });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/bulk-trash", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    ...
    if (!threadIds?.length)
      return res.status(400).json({ success: false, error: "No thread IDs" });
    const result = await bulkMoveToTrash(threadIds, email, crmUserId);
    res.json({ success: true, message: result.message, data: result });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.delete("/message/:id", protect, async (req, res) => {
  try {
    ...
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    await deleteEmail(req.params.id, email, crmUserId);
    res.json({ success: true, message: "Email deleted" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
// DRAFTS
// ═══════════════════════════════════════
router.post("/draft", protect, upload.array("attachments", 10), async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    ...
    const result = await saveDraft(
      ...
      cc || "", bcc || "", attachments, [], email, crmUserId
    );
    res.json({ success: true, data: result, message: "Draft saved" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
...
router.get("/drafts", protect, async (req, res) => {
  try {
    ...
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    const drafts = await getDrafts(parseInt(req.query.maxResults) || 20, email, crmUserId);
    res.json({ success: true, data: drafts, totalCount: drafts.length });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.get("/drafts/count", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    const drafts = await getDrafts(100, email, crmUserId);
    res.json({ success: true, count: drafts.length });
  } catch {
    res.json({ success: true, count: 0 });
  }
...
router.get("/draft/:draftId", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    const draft = await getDraft(req.params.draftId, email, crmUserId);
    res.json({ success: true, data: draft });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.delete("/draft/:id", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    await deleteDraft(req.params.id, email, crmUserId);
    res.json({ success: true, message: "Draft deleted" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
// ATTACHMENTS
// ═══════════════════════════════════════
router.get("/attachment/:messageId/:attachmentId", protect, async (req, res) => {
  try {
    const email = requireEmail(req, res);
    if (!email) return;
    const crmUserId = getCrmUserId(req);
...
// ═══════════════════════════════════════
// AUTH — these routes may be called before Gmail connection
// auth-url and oauth2callback are intentionally unprotected (OAuth flow)
// auth-status uses optional protect
// ═══════════════════════════════════════
  ...
  try {
    ...
    const redirectUri =
      host?.includes("localhost") || host?.includes("127.0.0.1")
        ? process.env.GMAIL_REDIRECT_URI
        : process.env.GMAIL_LIVE_REDIRECT_URI;
    // ✅ FIX: Embed crmUserId in OAuth state so callback knows which user
    res.json({ success: true, url: generateAuthUrl(redirectUri, crmUserId) });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
router.get("/auth-status", protect, async (req, res) => {
  try {
    const crmUserId = getCrmUserId(req);
    const email = req.query.email || null;
    const status = await checkAuth(email, crmUserId);
    res.json(status);
  } catch {
...
// ✅ oauth2callback reads crmUserId from OAuth state param
router.get("/oauth2callback", async (req, res) => {
  const { code, error, state } = req.query; // state = crmUserId
  const host = req.get("host");
  const isLocal = host?.includes("localhost") || host?.includes("127.0.0.1");
  const frontendUrl = isLocal
    ? process.env.FRONTEND_URL_LOCAL || "http://localhost:5173"
  ...
  // ✅ FIX: state param carries crmUserId through the OAuth redirect
  if (!state) {
    return res.redirect(
      `${frontendUrl}/emailchat?gmail_error=1&error=Missing+user+context`
    );
  }
...
router.delete("/disconnect", protect, async (req, res) => {
  try {
    ...
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    const result = await disconnectGmail(email, crmUserId);
    res.json(result);
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.get("/labels", protect, async (req, res) => {
  try {
    ...
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    const labels = await getLabels(email, crmUserId);
    res.json({ success: true, data: labels });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/watch", protect, async (req, res) => {
  try {
    ...
    if (!email) return;
    const crmUserId = getCrmUserId(req);
    const result = await watchInbox(email, crmUserId);
    res.json({ success: true, historyId: result.historyId });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.delete("/clear-cache", protect, (req, res) =>
  res.json({ success: true, message: "Cache cleared" })
);
