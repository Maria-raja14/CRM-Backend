...
async function getEmailForUser(req, res) {
  const crmUserId = getCrmUserId(req);
  if (!crmUserId) {
    res.status(401).json({ success: false, message: "Not authorized" });
    return null;
  }
  const email = await getConnectedEmail(crmUserId);
  if (!email) {
    res.status(400).json({
      success: false,
      message: "No Gmail account connected. Please connect Gmail first.",
    });
    return null;
  }
  return { crmUserId, email };
}

// ═══════════════════════════════════════════════════════════════════════════════
// PUBLIC ROUTES — NO JWT REQUIRED
// Only the OAuth callback must be public because Google redirects here without
// a Bearer token. The crmUserId is carried securely in the OAuth `state` param.
// ═══════════════════════════════════════════════════════════════════════════════

router.get("/oauth2callback", async (req, res) => {
  const { code, error, state: crmUserId } = req.query;
  ...
  try {
    ...
    res.redirect(
      `${frontendUrl}/emailchat?gmail_connected=1&email=${encodeURIComponent(
        result.email
      )}`
    );
  } catch (err) {
...
// protect middleware is applied per-route (not router.use) so the callback
// above remains public.
// ═══════════════════════════════════════════════════════════════════════════════
...
router.get("/accounts", protect, async (req, res) => {
  try {
    const crmUserId = getCrmUserId(req);
    const accounts = await getActiveAccountsForUser(crmUserId);
    res.json({ success: true, accounts, current: accounts[0] || null });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
router.get("/all-counts", protect, async (req, res) => {
  try {
    const userCtx = await getEmailForUser(req, res);
    if (!userCtx) return;
    const counts = await getLabelCounts(userCtx.crmUserId, userCtx.email);
    res.json({ success: true, counts });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
router.get("/all-threads", protect, async (req, res) => {
  try {
    const userCtx = await getEmailForUser(req, res);
    if (!userCtx) return;
    const threads = await listAllThreads(userCtx.crmUserId, userCtx.email);
    res.json({ success: true, data: threads });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.delete("/thread/:id", protect, async (req, res) => {
  try {
    const userCtx = await getEmailForUser(req, res);
    if (!userCtx) return;
    await deleteThread(req.params.id, userCtx.crmUserId, userCtx.email);
    res.json({ success: true, message: "Thread deleted" });
  } catch (err) {
    if (err.message?.includes("insufficientPermissions"))
...
router.post("/thread/:id/trash", protect, async (req, res) => {
  try {
    const userCtx = await getEmailForUser(req, res);
    if (!userCtx) return;
    await moveToTrash(req.params.id, userCtx.crmUserId, userCtx.email);
    res.json({ success: true, message: "Thread moved to trash" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/bulk-delete", protect, async (req, res) => {
  try {
    ...
    if (!threadIds?.length)
      return res.status(400).json({ success: false, error: "No thread IDs" });
    const result = permanent
      ? await bulkDeleteThreads(threadIds, userCtx.crmUserId, userCtx.email)
      : await bulkMoveToTrash(threadIds, userCtx.crmUserId, userCtx.email);
    res.json({ success: true, message: result.message, data: result });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/bulk-star", protect, async (req, res) => {
  try {
    ...
    if (!threadIds?.length)
      return res.status(400).json({ success: false, error: "No thread IDs" });
    const result = await bulkStarThreads(
      threadIds,
      star !== false,
      userCtx.crmUserId,
      userCtx.email
...
router.delete("/message/:id", protect, async (req, res) => {
  try {
    const userCtx = await getEmailForUser(req, res);
    if (!userCtx) return;
    await deleteEmail(req.params.id, userCtx.crmUserId, userCtx.email);
    res.json({ success: true, message: "Email deleted" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.get("/drafts", protect, async (req, res) => {
  try {
    ...
    const drafts = await getDrafts(
      parseInt(req.query.maxResults) || 20,
      userCtx.crmUserId,
      userCtx.email
    );
    res.json({ success: true, data: drafts, totalCount: drafts.length });
...
router.get("/drafts/count", protect, async (req, res) => {
  try {
    const userCtx = await getEmailForUser(req, res);
    if (!userCtx) return;
    const drafts = await getDrafts(100, userCtx.crmUserId, userCtx.email);
    res.json({ success: true, count: drafts.length });
  } catch {
    res.json({ success: true, count: 0 });
  }
...
router.delete("/draft/:id", protect, async (req, res) => {
  try {
    const userCtx = await getEmailForUser(req, res);
    if (!userCtx) return;
    await deleteDraft(req.params.id, userCtx.crmUserId, userCtx.email);
    res.json({ success: true, message: "Draft deleted" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
// Get auth URL — JWT required so we know which CRM user to embed in OAuth state
...
// Check auth status for this CRM user only
router.get("/auth-status", protect, async (req, res) => {
  try {
    ...
    const status = await checkAuth(crmUserId);
    res.json(status);
  } catch {
    res
      .status(500)
...
// Disconnect — only removes THIS user's Gmail token
router.delete("/disconnect", protect, async (req, res) => {
  try {
    ...
    const result = await disconnectGmail(crmUserId);
    res.json(result);
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.get("/labels", protect, async (req, res) => {
  try {
    const userCtx = await getEmailForUser(req, res);
    if (!userCtx) return;
    const labels = await getLabels(userCtx.crmUserId, userCtx.email);
    res.json({ success: true, data: labels });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/watch", protect, async (req, res) => {
  try {
    const userCtx = await getEmailForUser(req, res);
    if (!userCtx) return;
    const result = await watchInbox(userCtx.crmUserId, userCtx.email);
    res.json({ success: true, historyId: result.historyId });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
