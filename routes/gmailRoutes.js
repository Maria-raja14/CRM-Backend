...
// Gmail OAuth redirect URI – used by generateAuthUrl() and exchangeCodeForTokens()
const GMAIL_REDIRECT_URI = NODE_ENV === 'production'
...
console.log(`📧 Gmail routes running in ${NODE_ENV} mode`);
console.log(`📧 Frontend URL: ${FRONTEND_URL}`);
console.log(`📧 OAuth Redirect URI: ${GMAIL_REDIRECT_URI}`);
...
/**
 * Get Google OAuth URL
 * Uses the environment‑specific redirect URI
 */
router.get("/auth-url", (req, res) => {
  try {
    console.log("🔗 Generating auth URL...");
    // Pass the correct redirect URI to the service function
    const authUrl = generateAuthUrl(GMAIL_REDIRECT_URI);
    res.json({ success: true, url: authUrl });
  } catch (error) {
    console.error("❌ Error generating auth URL:", error);
    res.status(500).json({ success: false, error: error.message });
...
/**
 * OAuth2 callback
 * Exchanges code for tokens and redirects to frontend with the correct URL
 */
router.get("/oauth2callback", async (req, res) => {
  const { code, error } = req.query;
  ...
  try {
    console.log("🔄 Exchanging authorization code for tokens...");
    // Pass the correct redirect URI to the exchange function
    const result = await exchangeCodeForTokens(code, GMAIL_REDIRECT_URI);
    console.log("✅ Gmail connected successfully for:", result.email);
    res.redirect(`${FRONTEND_URL}/emailchat?gmail_connected=1&email=${encodeURIComponent(result.email)}`);
  } catch (err) {
    console.error("❌ OAuth failed:", err);
