...
import GmailToken from "../models/GmailToken.js"; // ✅ Default import — fixes "Cannot read properties of undefined (reading 'findOne')"
import {
  generateAuthUrl,
  exchangeCodeForTokens,
  listThreads,
...
// ─────────────────────────────────────────────────────────────────────────────
// KEY SECURITY FIX:
//   getEmail() previously read email from req.body / req.query / req.headers.
//   This let ANY logged-in (or even unauthenticated) user pass any email address
//   and read that person's mailbox.
//
//   Now:
//     • ALL routes are protected by the `protect` middleware (JWT verification).
//     • getEmail() reads the Gmail address that is stored against the
//       authenticated CRM user's account in GmailToken (linked via crm_user_id).
//     • We still accept req.query.email / req.body.email as a HINT, but we
//       verify that the resolved GmailToken actually belongs to req.user._id
//       before returning it.  See gmailService.getEmailForUser().
//
//   The only public routes are:
//     GET  /auth-url       – returns the Google OAuth URL (no mailbox data)
//     GET  /oauth2callback – Google redirects here after user consents
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Resolve the Gmail address for the currently-authenticated CRM user.
 *
 * Strategy (in order):
 *  1. If the request carries an email hint AND that GmailToken row has
 *     crm_user_id === req.user._id  → use it.
 *  2. Otherwise find any active GmailToken whose crm_user_id === req.user._id.
 *  3. If nothing is found → 403.
 *
 * This means one CRM user can only ever see their own connected Gmail account.
 */
async function resolveEmail(req, res) {
  const userId = req.user._id;

  // Hint from client (may be absent)
  const hint = (
    req.body?.email ||
    req.query?.email ||
  ...
  if (hint) {
    // Verify the hinted email actually belongs to THIS user
    tokenDoc = await GmailToken.findOne({
      email: hint,
      crm_user_id: userId,
      is_active: true,
    });
  ...
  if (!tokenDoc) {
    // Fall back: find any active Gmail account linked to this CRM user
    tokenDoc = await GmailToken.findOne({
      crm_user_id: userId,
      is_active: true,
  ...
  if (!tokenDoc) {
    res.status(403).json({
      success: false,
      message:
        "No Gmail account connected for your user. Please connect Gmail first.",
    });
    return null;
  }
...
// Returns the Google OAuth URL – no mailbox data exposed
router.get("/auth-url", (req, res) => {
  try {
    const host = req.get("host");
    const redirectUri =
...
// ═══════════════════════════════════════
router.use(protect); // ✅ Everything from here on needs a valid JWT

// ═══════════════════════════════════════
// LINK Gmail account to the CRM user
...
router.post("/link-account", async (req, res) => {
  try {
    ...
    // Make sure this token exists and is active
    const tokenDoc = await GmailToken.findOne({ email, is_active: true });
    if (!tokenDoc)
      return res.status(404).json({
        success: false,
        message: "Gmail account not found. Please connect first.",
      });

    // Bind it to the current CRM user
    tokenDoc.crm_user_id = req.user._id;
    await tokenDoc.save();

    res.json({
      success: true,
      message: `Gmail account ${email} linked to your profile.`,
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
router.get("/auth-status", async (req, res) => {
  try {
    ...
    if (!email) return; // resolveEmail already sent 403
    const status = await checkAuth(email);
    res.json(status);
  } catch {
    res
...
// ═══════════════════════════════════════
// ACCOUNTS  (only show accounts owned by this user)
// ═══════════════════════════════════════
router.get("/accounts", async (req, res) => {
  try {
    const accounts = await GmailToken.find({
      crm_user_id: req.user._id,
      is_active: true,
    }).sort({ last_connected: -1 });

    const result = accounts.map((a) => ({
      email: a.email,
      last_connected: a.last_connected,
    }));
    res.json({ success: true, accounts: result, current: result[0] || null });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
