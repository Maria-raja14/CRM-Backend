...
// ─────────────────────────────────────────────────────────────────────────────
// SECURITY HELPER
// Every protected route uses req.user._id (from JWT) to look up which Gmail
// account belongs to that CRM user. The caller CANNOT spoof another user's
// email – the email is always resolved server-side from the JWT identity.
// ─────────────────────────────────────────────────────────────────────────────
async function resolveEmail(req, res) {
  try {
    const email = await getConnectedEmailForUser(req.user._id);
    if (!email) {
      res.status(403).json({
        success: false,
        message: "No Gmail account connected for your user. Please connect Gmail first.",
      });
      return null;
    }
    return email;
...
// ═══════════════════════════════════════
// PUBLIC – Auth URL + OAuth callback
// (No JWT required – user isn't logged in to Gmail yet when these are called)
// ═══════════════════════════════════════

// Returns the Google OAuth URL. The CRM user id is embedded as `state` so we
// can link the Gmail token back to the correct CRM user on callback.
router.get("/auth-url", protect, (req, res) => {
  try {
    ...
    // Embed the CRM user id in the OAuth state parameter so the callback can
    // associate the Gmail token with the correct user.
    const url = generateAuthUrl(redirectUri, req.user._id.toString());
...
// The `state` param carries the CRM user id that was embedded in the auth URL.
router.get("/oauth2callback", async (req, res) => {
  ...
  if (!crmUserId) {
    return res.redirect(
      `${frontendUrl}/emailchat?gmail_error=1&error=Missing user state. Please try connecting again.`
    );
  }
...
router.get("/auth-status", protect, async (req, res) => {
  try {
    const status = await checkAuth(req.user._id);
    res.json(status);
  } catch {
...
// ── DISCONNECT ───────────────────────────────────────────────────────────────
// Only disconnects the Gmail account linked to the requesting CRM user.
...
// ── ACCOUNTS ─────────────────────────────────────────────────────────────────
// Returns only the Gmail account(s) connected to the requesting CRM user.
router.get("/accounts", protect, async (req, res) => {
  try {
    const accounts = await getAllActiveAccounts(req.user._id);
    res.json({ success: true, accounts, current: accounts[0] || null });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
