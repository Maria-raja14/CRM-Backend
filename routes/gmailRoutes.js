...
/**
 * gmail.routes.js  —  FIXED v2
 *
 * THE BUG: router.use(protect) was blocking ALL routes including
 * /auth-url, /auth-status, and /oauth2callback before they could run.
 *
 * THE FIX:
 *   - /oauth2callback  → truly public (Google calls it, no browser JWT)
 *   - /auth-url        → protect applied individually (CRM user must be logged in)
 *   - /auth-status     → protect applied individually (CRM user must be logged in)
 *   - everything else  → router.use(protect) after the auth routes
 *
 * SECURITY GUARANTEE: crmUserId ALWAYS comes from req.user._id (JWT).
 * The client never sends an email/userId to select whose mailbox to use.
 */
...
// SECURITY: crmUserId always comes from verified JWT, never from client
function getCrmUserId(req) {
  return req.user._id;
}

// ═══════════════════════════════════════════════════════════════════════════════
// ①  TRULY PUBLIC — Google OAuth callback (no CRM JWT, uses signed state)
// ═══════════════════════════════════════════════════════════════════════════════
router.get("/oauth2callback", async (req, res) => {
  ...
  if (!code)  return res.redirect(`${frontendUrl}/emailchat?gmail_error=1&error=No authorization code`);
  if (!state) return res.redirect(`${frontendUrl}/emailchat?gmail_error=1&error=Missing state parameter`);

  try {
    // exchangeCodeForTokens verifies the signed JWT state → extracts crmUserId
...
// ═══════════════════════════════════════════════════════════════════════════════
// ②  CRM-JWT REQUIRED (protect per-route) — auth management
//     These are called on EmailChat page load — the CRM user IS logged in
//     (their JWT is in localStorage) so protect works fine here.
// ═══════════════════════════════════════════════════════════════════════════════

// Generates Google OAuth URL with signed state embedding crmUserId
...
// Checks if this CRM user has Gmail connected
...
// ═══════════════════════════════════════════════════════════════════════════════
// ③  ALL REMAINING ROUTES — protected by global middleware below
// ═══════════════════════════════════════════════════════════════════════════════
router.use(protect);
...
router.post("/bulk-delete", async (req, res) => {
  try {
    const { threadIds, permanent = false } = req.body;
    if (!threadIds?.length) return res.status(400).json({ success: false, error: "No thread IDs" });
    const result = permanent
...
router.post("/send", upload.array("attachments", 10), async (req, res) => {
  try {
    ...
    for (const file of req.files || []) {
      if (file.size > 25 * 1024 * 1024)
        return res.status(400).json({ success: false, error: `"${file.originalname}" exceeds 25MB` });
      attachments.push({
        filename: file.originalname,
        content:  file.buffer.toString("base64"),
        mimetype: file.mimetype,
    ...
    let statusCode = 500;
    if (err.message?.includes("Invalid recipient") || err.message?.includes("25MB")) statusCode = 400;
    else if (err.message?.includes("auth")) statusCode = 401;
    res.status(statusCode).json({ success: false, error: err.message });
  }
});
...
router.post("/draft", upload.array("attachments", 10), async (req, res) => {
  try {
    ...
    if (!to) return res.status(400).json({ success: false, error: "Recipient required" });
    const attachments = (req.files || []).map((f) => ({
      filename: f.originalname, content: f.buffer.toString("base64"), mimetype: f.mimetype, size: f.size,
    }));
...
router.delete("/clear-cache", (_req, res) => res.json({ success: true, message: "Cache cleared" }));
