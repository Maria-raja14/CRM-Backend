...
// ─── Helper: read CRM user ID from verified JWT (set by protect middleware) ───
function getCrmUserId(req) {
  return req.user._id;
}

// ═══════════════════════════════════════════════════════════════════════════════
// ⚠️  PUBLIC ROUTES — must be declared BEFORE router.use(protect)
//
//  /oauth2callback  — Google redirects here after OAuth; no JWT exists yet
//  /test            — health check
// ═══════════════════════════════════════════════════════════════════════════════
...
router.get("/oauth2callback", async (req, res) => {
  ...
  // 🔒 Decode the CRM user ID that was embedded in the state param by /auth-url
  let crmUserId;
  try {
...
// ═══════════════════════════════════════════════════════════════════════════════
// 🔒  PROTECTED ROUTES — all routes below require a valid JWT Bearer token
// ═══════════════════════════════════════════════════════════════════════════════
router.use(protect);
...
router.get("/account", async (req, res) => {
  try {
    const account = await getConnectedAccount(getCrmUserId(req));
...
router.get("/auth-url", (req, res) => {
  try {
    ...
    const redirectUri = host?.includes("localhost") || host?.includes("127.0.0.1")
      ? process.env.GMAIL_REDIRECT_URI
      : process.env.GMAIL_LIVE_REDIRECT_URI;
    // 🔒 crmUserId is embedded in the OAuth state param
    const url = generateAuthUrl(redirectUri, getCrmUserId(req));
