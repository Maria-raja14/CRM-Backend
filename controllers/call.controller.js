...
// ── Derived from TWILIO_WHATSAPP_FROM=whatsapp:+13049443661 ──────────────────
// Strip "whatsapp:" prefix → "+13049443661"
const TWILIO_PHONE_NUMBER = (
  process.env.TWILIO_PHONE_NUMBER ||
  process.env.TWILIO_WHATSAPP_FROM?.replace("whatsapp:", "") ||
  ""
...
const BACKEND_URL = (process.env.BACKEND_URL || "https://uenjoytours.cloud").trim();

console.log(`📞 Twilio Voice Number : ${TWILIO_PHONE_NUMBER}`);
...
export const inboundCallWebhook = async (req, res) => {
  try {
    ...
    // ── Notify all CRM agents via WebSocket ───────────────────────────────────
    broadcast("call_incoming", {
      callSid:    CallSid,
      from:       From        || "",
      to:         To          || "",
    ...
    // ── TwiML response ────────────────────────────────────────────────────────
    // To forward inbound calls to a real agent, add to your .env:
    //   AGENT_PHONE_NUMBER=+919876543210
    // If not set, we play a message and hang up (call is still logged).
    const agentNumber = (process.env.AGENT_PHONE_NUMBER || "").trim();
...
<Response>
  ...
  <Dial
    record="record-from-ringing"
    recordingStatusCallback="${BACKEND_URL}/api/call/recording-callback"
    recordingStatusCallbackMethod="POST"
    action="${BACKEND_URL}/api/call/status"
    method="POST"
  >
    <Number>${agentNumber}</Number>
      ...
      twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Say voice="alice">Thank you for calling. Our team will get back to you shortly. Goodbye.</Say>
</Response>`;
    }
...
export const makeOutboundCall = async (req, res) => {
  try {
    ...
    const fromNumber = (from || TWILIO_PHONE_NUMBER).trim();
    if (!fromNumber)
      return res.status(500).json({
        error:
          "Twilio FROM number not configured. " +
          "Check TWILIO_WHATSAPP_FROM in your .env — current value should be: whatsapp:+13049443661",
      });

    // ── Normalise destination number ─────────────────────────────────────────
    let toFormatted = to.trim().replace(/[\s\-().]/g, "");
    if (!toFormatted.startsWith("+")) {
      // 10-digit Indian mobile (starts 6-9) → prefix +91
      if (/^[6-9]\d{9}$/.test(toFormatted)) toFormatted = "+91" + toFormatted;
      else toFormatted = "+" + toFormatted;
    }
    ...
    // ── KEY FIX ───────────────────────────────────────────────────────────────
    // Pass `to` as a query param so outboundTwiml can put it inside <Number>.
    // Without this the TwiML was <Number></Number> → Twilio fails immediately.
    const call = await twilioClient.calls.create({
      from:                          fromNumber,
      to:                            toFormatted,
      url:                           `${BACKEND_URL}/api/call/outbound-twiml?to=${encodeURIComponent(toFormatted)}`,
      statusCallback:                `${BACKEND_URL}/api/call/status`,
      statusCallbackMethod:          "POST",
      statusCallbackEvent:           ["initiated", "ringing", "answered", "completed"],
      record:                        true,
      recordingStatusCallback:       `${BACKEND_URL}/api/call/recording-callback`,
...
/* ═══════════════════════════════════════════════════════════════════════════════
   5.  OUTBOUND TWIML  —  GET /api/call/outbound-twiml
   ───────────────────────────────────────────────────
   KEY FIX: `to` comes from req.query (appended by makeOutboundCall).
   Before the fix this was always empty → <Number></Number> → Twilio failed
   with Status: Failed, Duration: 0s, no HTTP requests logged.
   ═══════════════════════════════════════════════════════════════════════════════ */
export const outboundTwiml = (req, res) => {
  const to = (req.query.to || "").trim();

  if (!to) {
    console.error("❌ outboundTwiml: `to` query param is missing");
    res.set("Content-Type", "text/xml");
    return res.status(200).send(
      "<Response><Say>No destination number was provided. Goodbye.</Say></Response>"
    );
  }

  console.log(`📋 TwiML for outbound → ${to}`);
