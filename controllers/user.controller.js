...
export default {
  ...
  getOperationsUsers: async (req, res) => {
    try {
      const users = await User.find()
        .populate("role", "name")
        .select("firstName lastName email role");

      const operationsUsers = users.filter(
        (u) => u.role?.name?.toLowerCase() === "operations"
      );

      res.json({ users: operationsUsers });
    } catch (error) {
  ...
  // ✅ NEW: Fetch only accounts users
  getAccountsUsers: async (req, res) => {
    try {
      const users = await User.find()
        .populate("role", "name")
        .select("firstName lastName email role");

      const accountsUsers = users.filter(
        (u) => u.role?.name?.toLowerCase() === "accounts"
      );

      res.json({ users: accountsUsers });
    } catch (error) {
