...
// //    Twilio posts here when a WhatsApp message arrives at our number
// // ─────────────────────────────────────────────────────────────────────────────
...
// // Enhanced helper: give a human-readable tip based on Twilio error code and message
// const getEnhancedTip = (err) => {
//   const msg = err.message || "";
//   const code = err.code || err.twilioCode;

//   if (code === 21408) {
//     return "The From phone number is not a valid Twilio WhatsApp sender. Check TWILIO_WHATSAPP_FROM in .env – it must be a WhatsApp-enabled Twilio number or the Sandbox number (e.g., whatsapp:+14155238886).";
//   }
//   if (code === 21211) {
//     return "The 'To' phone number is invalid. Please include the country code (e.g., +919361444764).";
//   }
//   if (code === 21610) {
//     return "The recipient is not opted in to receive WhatsApp messages from this sender. They must first send a message to your WhatsApp number.";
//   }
//   if (code === 63017) {
//     return "24-hour messaging window expired. Use a Template message instead.";
//   }
//   if (code === 63007) {
...
//     return "The recipient number is not verified. Add it in Twilio Sandbox settings (trial accounts only).";
//   }
//   if (msg.includes("outside of the window")) {
//     return "24-hour messaging window expired. Use a Template message instead.";
//   }
//   return null;
// };
...
// // ─────────────────────────────────────────────────────────────────────────────
// // 7. UPDATE CONVERSATION (rename contact / assign agent)
...
// ─── Helper: upsert conversation on every message ────────────────────────────
// BUG FIX: Previous version spread `update` (which conditionally included $inc)
// AND then added `$inc` again via the ternary — causing a MongoDB duplicate-key
// conflict on `$inc`. Now we build the update object cleanly in two branches.
const upsertConversation = async ({
  contactNumber,
  contactName,
  lastMessage,
  ...
  if (contactName) baseUpdate.contactName = contactName;

  // Build the final update object — never mix $inc with top-level fields
  // directly; use $set for clarity so MongoDB doesn't complain.
  const updateOp = incrementUnread
    ? { $set: baseUpdate, $inc: { unreadCount: 1 } }
    : { $set: baseUpdate };
...
export const inboundWebhook = async (req, res) => {
  try {
    ...
    // BUG FIX: Notify all connected users (correct for a shared CRM inbox).
    // The frontend is responsible for only showing unread badges for
    // conversations that are NOT currently active on that user's screen.
    Object.keys(connectedUsers).forEach((uid) => {
      notifyUser(uid, "whatsapp_new_message", payload);
    });
...
export const sendMessage = async (req, res) => {
  try {
    ...
    // Normalise once here — twilio.service no longer re-normalises
    const toFormatted = normaliseWhatsappNumber(to);
...
// ─────────────────────────────────────────────────────────────────────────────
// Enhanced helper: human-readable tip based on Twilio error code / message
// ─────────────────────────────────────────────────────────────────────────────
const getEnhancedTip = (err) => {
  const msg = err.message || "";
  const code = err.code || err.twilioCode;
...
export const sendTemplate = async (req, res) => {
  try {
    ...
    // BUG FIX: also broadcast template sends so all connected agents see it
    const payload = {
      _id: message._id,
      contactNumber: toFormatted,
      direction: "outbound",
...
export const sendMedia = async (req, res) => {
  try {
    ...
    // BUG FIX: broadcast media sends too
    const payload = {
      _id: message._id,
      contactNumber: toFormatted,
      direction: "outbound",
