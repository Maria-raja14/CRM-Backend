...
export default {
  ...
  createInvoice: async (req, res) => {
    try {
      ...
      //   return res.status(403).json({ error: "Only Admin or Manager can create invoices" });
      // }


// ✅ ADD accounts here
if (roleName !== "admin" && roleName !== "manager" && roleName !== "accounts") {
  return res.status(403).json({ error: "Only Admin, Manager or Accounts can create invoices" });
}
  ...
  //    admin / manager → see ALL invoices
  //    sales           → see invoices assigned to them
  //    accounts        → see invoices assigned to them
  getAllInvoices: async (req, res) => {
    try {
      if (!req.user) return res.status(401).json({ error: "Unauthorized: No user found" });
      ...
      if (roleName === "admin" || roleName === "manager") {
        ...
        // Sales sees only their assigned invoices
        invoicesQuery = Invoice.find({ assignTo: req.user._id });
      } else if (roleName === "accounts") {
        // ✅ Accounts sees only invoices assigned to them
        invoicesQuery = Invoice.find({ assignTo: req.user._id });
      } else {
        return res.status(403).json({ error: "Access denied" });
      }
